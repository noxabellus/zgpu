//! Manages a single, dynamically-growing texture array.
...
pub fn flush(self: *MultiAtlas, context: ProviderContext) !bool {
...
fn createTextureArray(self: *MultiAtlas, layer_count: u32) !void {
...
fn growTextureArray(self: *MultiAtlas, new_layer_count: u32) !void {
...
fn uploadImage(self: *MultiAtlas, image: Atlas.InputImage, x: u32, y: u32, layer: u32) !void {
...
fn generateMipChain(self: *MultiAtlas, source: Atlas.InputImage, max_levels: u32) ![]const Atlas.InputImage {
    if (max_levels <= 1) {
        const slice = try self.allocator.alloc(Atlas.InputImage, 1);
...
pub fn debugWriteAllAtlasesToPng(self: *MultiAtlas, base_filename: []const u8) !void {
    ...
    for (0..self.pack_contexts.items.len) |i| {
        ...
        try Atlas.debugWriteLayerToPng(self.device, self.queue, self.allocator, self.texture, @intCast(i), self.atlas_width, self.atlas_height, filename);
    }
}
